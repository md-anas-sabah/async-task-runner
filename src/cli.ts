...
/**
 * CLI Interface for async-task-runner
 * 
 * Allows running tasks from command line with file or inline JSON input.
 * Supports all configuration options through CLI flags.
 */
...
interface CLIConfig {
  tasks: CLITaskDefinition[];
  options?: TaskOptions;
}

// Convert CLI task definitions to executable functions
function createTaskFromDefinition(taskDef: CLITaskDefinition): () => Promise<any> {
  return async () => {
    // HTTP request task
    if (taskDef.url) {
...
async function loadTasksFromFile(filePath: string): Promise<CLIConfig> {
  try {
...
async function runTasksFromCLI(config: CLIConfig, cliOptions: any): Promise<void> {
  // Merge file options with CLI options (CLI takes precedence)
  ...
  if (cliOptions.verbose) {
    ...
    if (successfulResults.length > 0) {
      ...
      successfulResults.forEach((result, index) => {
        ...
        if (taskResult.status) console.log(`      Status: ${taskResult.status} ${taskResult.statusText}`);
        if (taskResult.exitCode !== undefined) console.log(`      Exit code: ${taskResult.exitCode}`);
      });
    }
  ...
  .action(async (options) => {
    try {
      ...
      if (options.file) {
        ...
        config = JSON.parse(options.json) as CLIConfig;
      } else {
